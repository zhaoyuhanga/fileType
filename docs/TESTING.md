...
| 墨软文档 | 添加 Word/PDF/Excel/MD → 双击打开与编辑保存 → 一键美化（模板/目录/表格）→ 填充计算 → 与另一文档合并（看报告与差异，点「应用到文档」后保存应当是另存为）→ AI 循环美化（无模型时走本地规则）→ 脱敏与水印 |
| 墨软文档 · 图片与打印 | 打开一份**带插图**的 Word → 查看编辑页能看到图（预览里内嵌）→ 保存后再打开图片仍在；「打印」PDF 时能选打印机/份数/页范围；导出 TXT/MD 时提示"图片以占位文字保留" |
| 墨软文档 · 设置接线 | 设置→文档：改「版本保留」后多存几版看快照数量；改「AI 循环美化默认值」「默认美化参数」后重进板块看面板初值；勾上「每轮结束人工确认」跑循环应当每轮停下来问；**取消**「自动保存」「允许云端上传」后确认真的关掉了（注册表里存的是字符串 "false"，踩过 `bool("false") is True` 的坑）；改完设置回到美化/循环页确认没把模板字体字号冲掉 |
| 通用 | 设置对话框六个分区可保存；切换板块无卡顿；关闭应用不残留后台线程 |
