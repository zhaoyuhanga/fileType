...
- **组件库** `ui_kit/components/`：`ColumnPage`（页边距 24、区块间距 16）、`PageHeader`、
  `SectionCard`、`EmptyState`（空列表必须给下一步操作）、`Toolbar`、`chip`/`divider`/
  三种按钮层级 —— 页面不再各写各的留白。
- **首页重排**：页头 + 区块卡片；板块卡改响应式栅格（2~4 列，按窗口宽度重排），
  卡片等高、最后一行跨列铺满，消除"右侧大片空白/卡片高低不齐"。
- **验收工具** `packaging/ui_snapshot.py`：把各板块页面离屏渲染为 `docs/ui/board-*.png`，
  便于人眼检查排版（离屏环境自动指向系统字体目录，截图文字可读）。
- 规范文档 `docs/UI_GUIDE.md`（三条硬性规则 + 令牌表 + 组件表 + 页面模板 + 截图检查清单）；
  测试 `tests/test_ui_design_system.py`（圆角下限、间距/字号有序、深浅主题字段齐全、
  组件行为、首页栅格响应式与末行铺满）；`tests/conftest.py` 提供公共 `qapp` 夹具。
- 全量 pytest 通过。
